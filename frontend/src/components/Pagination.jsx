...
export const Pagination = () => {
    ...
    const getPreviousButton = () => {
        if((homes.nextPage-1) <= 0) {
            return <Button attributes={{disabled:false}} classes={"cursor-not-allowed !bg-blue-400"}>Prev</Button>
        }else {
    ...
    const getNextButton = () => {
        if(homes.nextPage >= homes.totalPages)  {
            return <Button attributes={{disabled:false}} classes={"cursor-not-allowed bg-blue-400"}>Next</Button>
        }else {
